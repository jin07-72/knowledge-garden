...
body {
  background:
    radial-gradient(circle at 85% 8%, rgba(139, 213, 202, 0.06), transparent 28rem), var(--light);
}
...
export async function checkBuiltLinks(root) {
  ...
  for (const file of await filesUnder(root)) {
    ...
    for (const href of hrefs) {
      ...
      if (!pathPart || /\.(?:css|js|png|jpe?g|gif|svg|webp|xml|ico|woff2?)$/i.test(pathPart))
        continue
